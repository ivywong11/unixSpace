...
{
	printk("drivers open\n");
	//open的时候，会得到inode编号->获取到节点信息->填充到内存inode结构中
	switch(MINOR(inode->i_redev))
	{	
		//将inode中的设备次设备号取出，根据次设备号来区分
		//将不同次设备号的FIFO的地址放到对应file结构的private_date成员中，这样在read的时候，从file中取出，可以读取对应的FIFO
		default:
		case 0:
			filp->private_data = &j_3_5_1_fifo;
			break;
		case 1:
			filp->private_date = &j_3_5_2_fifo;
			break;	
	}
	return 0;
}
